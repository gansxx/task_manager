...
- `settings.ts`：设置页实现与默认设置。当前包括监听目录、开始/完成 token 格式、时间戳精度、即时归档和语言设置。
...
- `archive-service.ts`：定义 SQLite-only 归档位置；归档不再创建或修改任何归档 Markdown 文件。
- `task-completion-store.ts`：初始化嵌入式 SQL WASM、管理插件 SQLite 数据库，并保存完成和归档记录。
...
4. 默认处理器根据设置写入 `@start(...)`、`@done(...)`，取消完成时移除 `@done(...)`；即时归档打开时将任务写入 SQLite 并删除原行。
5. `TaskArchiveService` 将归档目标标记为 SQLite；`TaskCompletionStore` 保存任务原文、来源、完成时间和归档时间。
...
- 设置页提供“隐藏任务元数据标签”开关；默认关闭，开启后会在阅读模式和实时阅览模式隐藏 `@start(...)`、`@done(...)`、`@priority(...)`、`@from(...)`、`@archived(...)` 等行内标签。

## 任务面板体验增强

- 任务侧边栏会保留最近一次扫描结果作为会话缓存，打开面板时先展示缓存，再异步刷新，避免刚打开时出现空白任务列表。
- 全仓库或较大范围扫描期间，侧边栏顶部与状态栏会显示 loading 提示。
- 侧边栏新增归档状态过滤，可筛选全部任务、未归档任务或已归档任务。
- 任务卡片支持右键菜单，可直接修改优先级或追加 `@comment("...")` 评论。
- 隐藏元数据标签默认关闭；开启后仅在阅读模式隐藏标签，编辑器中仍保留可编辑标签，避免影响继续打标签。
- 手动归档确认框支持“以后不再提示”，勾选后会写入跳过归档确认设置。
- 任务侧边栏按 Markdown 缩进建立父子层级，并提供类似预览树的逐级展开/折叠。
- 评论不再写入父任务行内，而是写成缩进一级的 `- comment @comment` 子项，并在侧边栏、阅读模式和实时预览中使用评论样式。
- 任务过滤栏现在可以折叠；Archive 过滤默认显示 Not archived，以避免默认任务面板混入已归档任务。
- File path 区域新增文件夹下拉选择，便于快速限定任务扫描范围。
