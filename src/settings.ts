...
export class TaskManagerSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      .setName(copy.scanVaultOnStartupName)
      .setDesc(copy.scanVaultOnStartupDesc)
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.scanVaultOnStartup)
          .onChange(async (value) => {
            this.plugin.settings.scanVaultOnStartup = value;
            await this.onSave();
          }),
      );
