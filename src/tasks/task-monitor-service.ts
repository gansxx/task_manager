...
export class TaskMonitorService {
  ...
  async setTaskCompletion(file: TFile, lineNumber: number, checked: boolean): Promise<boolean> {
    ...
    if (!checked) {
      ...
      if (completed) {
        await this.completionStore.recordCompletion(file.path, lineNumber, updatedLine, completed);
      }
    }
    ...
    const archived = await this.archiveService.archiveCompletedTask(
      file,
      archiveBlock.text,
      getCurrentTimestamp(this.getSettings().timestampPrecision),
    );
    await this.completionStore.recordArchive(
      file.path,
  ...
  registerDefaultHandlers(): void {
    ...
    this.pipeline.on("taskCompleted", async (event) => {
      ...
      const completed = parseTaskLine(archivedLine);
      if (completed) {
        await this.completionStore.recordCompletion(
          event.file.path,
          event.lineNumber,
          archivedLine,
          completed,
  ...
  private async refreshTaskLines(
    ...
    for (let lineNumber = 0; lineNumber < lines.length; lineNumber += 1) {
      ...
      if (!parsed.checked && parsed.doneToken) {
        await this.completionStore.markReopened(file.path, lineNumber, line);
      ...
      if (!parsed.doneToken) {
        ...
        if (nextLine !== line) {
          lines[lineNumber] = nextLine;
          updated = true;
          const completed = parseTaskLine(nextLine);
          if (completed) {
            await this.completionStore.recordCompletion(file.path, lineNumber, nextLine, completed);
          }
        }
      }
